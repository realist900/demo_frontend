...
class _CatScreenState extends State<CatScreen> {
  ...
  void initState() {
    ...
    _catController.loadInitial();
    scrollController.addListener(() {
      if (scrollController.position.pixels == scrollController.position.maxScrollExtent) {
        _catController.loadMore();
      }
    });
    context.dispatchNotification(RequestFirebaseTokenNotification());
  }
      ...
      body: ListView.separated(
        ...
        itemBuilder: (context, index) {
          return CatListItem(
            ...
            onTapEdit: (cat) async {
              final navigator = Navigator.of(context);
              final bool? res = await navigator.push<bool>(MaterialPageRoute(builder: (_) => EditCatScreen(cat: cat)));
              if (res ?? false) {
                _catController.loadInitial();
              }
            },
